...
from components import tab_forecast, tab_weather, tab_models
from components import tab_generation, tab_alerts, tab_simulator, tab_backtest
        ...
        html.Div([
            dbc.Row([
                dbc.Col([
                    ...
                    html.P("Demand Forecasting & Analytics", className="dashboard-subtitle"),
                ], md=4, className="d-flex flex-column justify-content-center"),

                dbc.Col([
                    html.Label("Region", style={"color": "#8a8fa8", "fontSize": "0.7rem"}),
                    dbc.Select(
                        id="region-selector",
                        options=[{"label": name, "value": code} for code, name in REGION_NAMES.items()],
                        value="FPL",
                ...
                dbc.Col([
                    html.Label("Persona", style={"color": "#8a8fa8", "fontSize": "0.7rem"}),
                    dbc.Select(
                        id="persona-selector",
                        options=[
                            {"label": f'{p["avatar"]} {p["title"]}', "value": p["id"]}
                ...
                dbc.Col([
                    html.Div([
                        ...
                        html.Div(id="header-freshness", style={"textAlign": "right"}),
                    ], className="d-flex flex-column align-items-end justify-content-center h-100"),
                ], md=3),
            ], align="center"),
        ], className="dashboard-header", id="dashboard-header"),

        # ── Bookmark notification ──────────────────────────────
        html.Div(id="bookmark-toast", style={"position": "fixed", "top": "10px",
                  "right": "10px", "zIndex": 9999}),
        ...
        dbc.Row([
            dbc.Col(
                ...
                dbc.Tabs(
                    ...
                    children=[
                        dbc.Tab(tab_forecast.layout(), label=TAB_LABELS["tab-forecast"], tab_id="tab-forecast"),
                        dbc.Tab(tab_demand_outlook.layout(), label=TAB_LABELS["tab-outlook"], tab_id="tab-outlook"),
                        dbc.Tab(tab_backtest.layout(), label=TAB_LABELS["tab-backtest"], tab_id="tab-backtest"),
                    ],
                ),
                md=9,
            ),
