...
| Cloud Run Jobs cost (us-east1) | **$0.000018 / vCPU-second**, **$0.000002 / GiB-second** (always-allocated tier — Jobs always bill this way, and so does a Service running `--no-cpu-throttling`). Scoring **measured $59/mo** at the 927s 7-day mean (22,244 task-s/day, n=168, 4 vCPU / 8 GiB); **at the post-vectorisation 370.7s that becomes ~8,900 task-s/day ≈ $24/mo — UNCONFIRMED, n=1**, needs a day of ticks before it goes in a budget. Training **~$60/mo** (~25,000 task-s/day at 4 vCPU / 4 GiB). **An earlier row estimated scoring at ~$27–36 and was low by ~1.6x** because it assumed ~700s; its "training is 3x scoring" claim died with it | `gcloud run jobs executions list` durations × Cloud Billing Catalog rates, 2026-08-05. Training figure is from pre-CV-fix executions at current 4 GiB sizing, so it is an upper bound |
