...
| What `forecast` actually spends 60.1% on | **Unknown, and now instrumented rather than inferred.** Local profiling with the real SPA pickles pulled from GCS (720h horizon, 46-feature booster, 6000 trees) puts the ENTIRE predict path at **3.35 s/BA**: `_build_future_feature_frame` 1,380 ms (41%), XGBoost recursive 384-step 1,001 ms (30%), Prophet 769 ms (23%), SARIMAX incl. Kalman gap-advance 203 ms (6%). Production's `forecast` phase measures **~60 s/BA** (80.4 s for SPA) — an **~18× gap** that none of these components explain, even allowing ~3× for slower Cloud Run cores. The largest untested suspect is `_overlay_weather_forecast`, which the local harness skips (`weather_df=None`) and which ADR-012 feeds up to 12 points per BA. **Do not optimise this phase from a laptop** — two targets were chosen and discarded that way. Sub-phase timing now ships in `scoring_phase_rollup.forecast_substeps`; one production tick settles it | Local profile 2026-08-05 against `gs://…/models/SPA/{xgboost,prophet,arima}` at `latest.json`; phase totals from `scoring_phase_rollup` |
| Single-row XGBoost predict input format | Passing a **pandas DataFrame** to `predict` costs ~1.2 ms/call against ~0.2 ms for numpy — the pandas→DMatrix marshalling dwarfs tree traversal, and going 400 → 6000 trees moves DataFrame-input only 477 → 541 ms per 384 steps. On the real SPA booster under **8 concurrent workers**, the recursive path measures **836 ms/BA** (DataFrame, `n_jobs=-1`) vs **503** (numpy/`inplace_predict`) vs **483** (numpy + `n_jobs=1`) — a real but modest **1.7×**, worth ~350 ms/BA or **~0.35% of summed worker time**. Recorded because it is a standing trap, not because it is worth a PR alone | Benchmarks 2026-08-05 on the persisted SPA booster; `n_jobs=-1` is baked in at `models/xgboost_model.py:38` |
...
| Artifact Registry (`portfolio` repo) | **303.2 GiB / 406 image versions** at ~$0.10/GB/mo = **~$32/mo**, from ~750 MB per CI deploy with nothing removing them. A cleanup policy (`delete-stale` >30d + `keep-recent-versions` 20) existed but sat in **`cleanupPolicyDryRun: true`** — evaluating and logging while deleting nothing. Activated 2026-08-05: **284 versions / ~212 GiB / ~$21/mo** reclaimed, 122 versions (~91 GiB) retained. Rollback is now limited to ~30 days of deploys | `gcloud artifacts repositories describe/versions list`, read 2026-08-05; the live image was verified protected by `keep-recent-versions` before activating (tag resolved to digest — `versions list` returns digests, so a tag-vs-digest comparison silently never matches) |
