...
**Sizing it honestly:** this is a **`fetch`-phase reclaim, not a fix for the 2026-08-04 incident.** An earlier reading blamed ADR-012 for making this leg ~12× heavier and causing the runtime alert; the runtime record refutes that (daily medians are flat across the 07-22/07-23 flips, and the incident was EIA retry tax). `fetch` is 13.0% of worker time; the archive leg's share of that is **estimated, not measured** (~50–70s summed, ~1%), so the saving stays unverified until a before/after `scoring_phase_rollup` sizes it.
