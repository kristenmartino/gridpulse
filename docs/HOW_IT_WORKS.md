...
**HTTP surfaces on the web tier:** the Dash UI, `/about`, `/methodology`,
`/coverage` and `/benchmark` (public pages served from `web/`),
`/health` (+ `?deep=1`), `/metrics`,
`/robots.txt`, `/sitemap.xml` and `/BingSiteAuth.xml` (generated by
`seo.py`), and
...
a route that read it would 404 everywhere that matters. The cost is drift,
and `tests/unit/test_methodology_page.py` pays it down by asserting every
figure the page publishes is still present in the doc that sources it.

**`/coverage` takes the opposite choice, and for the same reason.** Its 51-row
table is generated at request time from `config.REGION_NAMES`, `STATE_TO_BA`,
`REGION_CAPACITY_MW` and `PEAK_DERIVED_CAPACITY` — `config.py` *does* ship in
the image, so rendering from it makes disagreement with what the product
actually covers structurally impossible. A committed copy of a 51-row list
would be stale within a release. Both routes fail open: a render error leaves
the marker empty and the surrounding prose stands on its own.
