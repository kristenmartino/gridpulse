...
### 36. "Tell me about a time the result got better and you still said no."

**Situation**: I'd re-run a pre-registered study after fixing a defect in my own treatment arm. The first run had come back against its hypothesis, and I'd traced that to my fix zero-filling an impossible demand value on 13% of forecast steps. The re-run was a new pre-registration — same seed, same injected gaps in the same places, so the two runs were paired and only the policy had moved.

**Task**: Run it once, per the stopping rule, and decide the feature flag.

**Action**: The diagnosis was vindicated more cleanly than I expected. On the population the fix targets, the point estimate moved from **−0.265 to +0.275** — a half-point swing, with mean and median now agreeing in sign where they had contradicted each other. The other stratum moved from +0.187 to +0.620. Every one of the nineteen balancing authorities was positive in direction. Both strata now cleared the magnitude test they'd been underpowered for.

And I still reported it as **not confirmed**, because our evaluation policy doesn't decide on averages alone. It also requires the change to win 75% of individual windows, and these won 61% and 74%. Large on average, unreliable per window — a different failure than the first run's, and still a failure.

The 74% is the part I want to be judged on. That's **321 windows out of 432 against a bar of 324**. Three windows. I had the raw artifacts, I could have re-run with a different window count or horizon and crossed it inside an hour, and nobody would have known to check. Instead I wrote the shortfall into the study document explicitly — with the arithmetic, so any reader can do the division themselves — and stated that it is not quoted as a pass anywhere. The threshold was fixed before the run precisely so that a near-miss couldn't be renegotiated by the person who wanted it to pass.

I also refused a subtler temptation. The pre-registration listed four possible outcomes with their readings fixed in advance, and the one that applied — "both inconclusive" — came with a stated *rationale* that turned out to be wrong: it predicted the effect would be too small to matter, and the effect was actually larger than the detection threshold in both strata. The honest move was to keep the pre-committed reading and say plainly that the mechanism inside it differed from what I'd anticipated. Writing a fifth reading to describe "large but unreliable" would have been choosing the interpretation after seeing the data, which is the entire thing pre-registration exists to prevent.

One more thing fell out of it. To satisfy a criterion requiring the zero-fill to be *re-measured in the run* rather than assumed from the earlier fix's own reporting, I instrumented the arm — and gave the instrument a control designed to disagree, because the criterion is satisfied by a zero and a broken probe reports the same zero. That control channel then flagged 14 steps I wasn't looking for: a **second, unrelated defect**, where the history object reserves room for the model's own predictions measured from the last data point it actually has. When the upstream's tail is missing — exactly the case this whole feature exists for — the reservation runs short and the recursion silently discards its own later predictions. I did not fix it mid-analysis; that's the same error I'd already made once. I documented it, filed it, and flagged that a related shadow instrument shares the arm and would inherit it.

**Result**: Flag stays off. It also does *not* get removed — removal was the pre-committed response to the estimate staying negative, and it hadn't. The published answer is "the effect is real, positive, directionally consistent across every BA, and not reliable enough per window to ship," plus a correctness bug found by a control I'd added for a different reason.

**Lesson to convey**: *Pre-registration is cheap to write and only costs you something on the day it tells you no — which is the day it's worth anything. The tell isn't whether you accept a negative result; it's what you do three windows from the threshold with the raw data on your laptop. Two habits carried the weight: publishing the near-miss arithmetic so the reader can check the division, and giving every "we measured zero" a control that proves the instrument can report non-zero — that second one is what turned a compliance checkbox into finding a bug nobody was looking for.*
