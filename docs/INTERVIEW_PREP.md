...
Follow-on (the second layer of camouflage): with the charts finally rendering, the residuals came up *dead flat at zero* — a single histogram bin, an empty error-by-hour. The predictions (10k–28k MW) and SHAP were obviously real, so why were residuals exactly zero? The diagnostics writer in the scoring job computed `residuals = actual − ensemble`, but sourced the ensemble via `get_forecasts(...).get("ensemble", actual_demand)`. That `get_forecasts` re-loads models through a *local-pickle* path the Job container never populates (it loads from GCS); post-#149 it returned `unavailable` with no `"ensemble"`, so the `.get(..., actual_demand)` default used **the actuals as the prediction** → `actual − actual = 0`. A fabricated "perfect model." Fixed by scoring the XGBoost model the job already held, in-sample over the engineered history, and by *skipping* the write entirely when no real model is available rather than defaulting to actuals (closed #166).

**Lesson to convey**: *`is` is not `==`. An identity check on a value that's reconstructed on every callback is a time bomb — it works in the one test that passes the sentinel object and fails everywhere real. And removing fake fallbacks is double-edged: it makes you honest, but it also strips the camouflage off every latent bug the fake data was quietly covering — here, twice in a row: the honesty fix exposed the dead view, and fixing the view exposed the fabricated zero-residuals behind it. A silent `dict.get(key, fallback)` where the fallback is "the answer itself" (actual-as-prediction) is the same anti-pattern as the identity check — both fail invisibly. Budget for the chain of bugs an honesty fix will surface.*
