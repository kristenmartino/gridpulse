...
| `benchmark_scoreability_alert.json` | **Job tier (2026-08-18, #535).** Log-based (`jsonPayload.event="benchmark_scoreability_drop"` or `"benchmark_coverage_at_risk"`) — fires when the public `/benchmark` scorecard drops below `BENCHMARK_MIN_SCOREABLE` BAs, or when a still-scoreable BA's `df_coverage` enters the warning band above the 0.80 gate. The scorecard published **25 of 51** for ~3 weeks with five of seven large ISOs missing from its fleet medians, while the job succeeded and exited 0 — the headline changed the population it described and nothing said so. Counts the *failing* direction deliberately: a threshold-below on the existing `benchmark_fleet_written` count would go quiet exactly when the phase stopped emitting. |
