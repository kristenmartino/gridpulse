...
class TestKbdChipsAreNotShipped:
    ...
    def test_kbd_hint_class_absent_from_css(self):
        css = _read_css()
        assert ".gp-header__kbd-hint" not in css, (
            "Kbd shortcut chips were intentionally not shipped — see "
            "PR #89 / issue #23 closure rationale."
        )

    def test_shortcut_group_class_absent_from_css(self):
        css = _read_css()
        assert ".gp-header__shortcut-group" not in css

    def test_kbd_chips_absent_from_layout(self):
        """The two <kbd> chips that wrapped region/persona selectors
        in earlier drafts must not render in the header."""
        ...
        def walk(node):
            ...
            if "gp-header__kbd-hint" in cls or "gp-header__shortcut-group" in cls:
                kbd_count += 1
