...
- *(Bookkeeping, refreshed 2026-08-11: **#452 now has its entry** — folded into the "#273 CLOSED" decision above, which carries its P2-17 hysteresis finding, so the placeholder that stood here is retired. **The one now missing is [#473](https://github.com/kristenmartino/gridpulse/pull/473)** (merged 2026-08-11T01:02Z): it ran the [#451](https://github.com/kristenmartino/gridpulse/issues/451) pre-registered A/B without the training runs it was parked for — the arms differ only in the weight vector, so the expensive half is paid once — and landed EWMA-smoothed holdout MAPE as the ensemble-weight input behind `smoothed_ensemble_weights`, **off**, with the series persisted regardless of the flag so a later flip finds real history. α=0.3 beat α=0.5 by roughly 2× (+0.524 vs +0.271 WAPE pts). Left for its author rather than summarised third-hand. **The flag was never flipped and #451 is now closed on "outcome 2 — ships nothing":** the 51-BA re-measure held the WAPE win but the bias check did not, and a failed satisficing constraint vetoes a win per `docs/EVALUATION_POLICY.md`. The bias half is carved out as [#478](https://github.com/kristenmartino/gridpulse/issues/478) — a shadow scoring pass rather than per-arm training runs. See the #451 decision above; `gh pr list --state merged` remains the authoritative set.)*
