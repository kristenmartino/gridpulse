...
- **2026-08-05** **#414 reduced to the archive cache alone, and my own diagnosis retracted.** The concurrent-session PR opened against the `scoring_runtime_creep` alert carried two halves. Its **instrumentation half was already on main** — shipped here rather than rewritten — so keeping it would have been a duplicate, and the PR was additionally un-mergeable against 54 commits of drift. Its **causal claim was wrong**: it attributed the alert to ADR-012 making the ERA5 archive leg ~12× heavier, and the runtime record refutes that (flat daily medians across the 07-22/07-23 flips; the incident was EIA retry tax under partial degradation). What survives is the cross-run GCS cache for the archive segment, now sized honestly against the instrumentation that did land: `fetch` is **13.0%** of worker time and the archive leg's share of it is **estimated, not measured** (~50-70s summed, ~1%) — real, unverified until a before/after `scoring_phase_rollup`, and **nowhere near #171's remaining 68s on its own**. The design worth keeping is that **coverage decides a cache hit, not a TTL**: the frame must span the days requested, so a moved window misses, which makes it structurally incapable of serving a window other than the one asked for. Fail-open at every seam, flag `weather_archive_cache`. The refuted diagnosis is corrected in the code comment, `HOW_IT_WORKS`, the flag comment and on #389 itself, so the wrong story is not left recoverable from any of them. 3023 tests green. [this PR]
