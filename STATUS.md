...
1. **Phase 3 of `prod-readiness`** (production safety, ~7h): [#149](https://github.com/kristenmartino/gridpulse/issues/149) strict prod fallback gating in `model_service` (no simulated values when `REQUIRE_REDIS`) — **start here, highest trust risk**, [#150](https://github.com/kristenmartino/gridpulse/issues/150) Prophet interval honesty (the 95% is heuristic), [#155](https://github.com/kristenmartino/gridpulse/issues/155) LDWP/drift robust sMAPE for low-demand regions (closes the [#142](https://github.com/kristenmartino/gridpulse/issues/142) bug report).
