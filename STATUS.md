...
**The issue is still OPEN and deliberately not closed here:** closing a
15-item tracking issue is a judgement about all 15, not about the four this
pass verified, and #452 explicitly split its own scope — the weights half is
**not** decided, pre-registered as an A/B in
[#451](https://github.com/kristenmartino/gridpulse/issues/451). Someone should walk #273's checklist and
close it deliberately. **Slot 1 is therefore no longer the top of this block
on merit**, but re-ordering the numbered slots below needs a judgement this
backfill has no basis for. Slot 3 ([#275](https://github.com/kristenmartino/gridpulse/issues/275)) was not
re-verified in either pass and still reads 20 of 23 from 08-05.)*
...
- *(Still missing an entry: **[#452](https://github.com/kristenmartino/gridpulse/pull/452)** (`42d3ed9`, P2-17 — merged 2026-08-10T18:17Z, while this backfill was in review). It closes slot 1 and carries its own finding: the issue prescribed **smoothing**, 1,620 recorded holdout MAPEs rejected it, and it ships **hysteresis on the gate** instead, with the weights half deliberately undecided and pre-registered as [#451](https://github.com/kristenmartino/gridpulse/issues/451). Left for its author rather than summarised third-hand from the PR body. **#438's entry landed** via [#450](https://github.com/kristenmartino/gridpulse/pull/450) and is below. `gh pr list --state merged` remains the authoritative set — this log has now gone stale twice in one day while being written, which is the argument for checking it rather than reading it.)*
